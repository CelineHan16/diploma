...
export const checkout = createAsyncThunk('cart/checkout', async (order, thunkAPI) => {
  const response = await axios.post('https://cart-e5935-default-rtdb.firebaseio.com/orders.json', order);
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    restore: (store, action) => {
      store.items = JSON.parse(localStorage.getItem('cartItems') ? localStorage.getItem('cartItems') : '{}');
    }
  },
  extraReducers: {
